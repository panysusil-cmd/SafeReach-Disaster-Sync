...
export default function Sidebar({ selectedIncident, onUpdateStatus }) {
  ...
  const getStatusColor = (status) => {
    if (status === 'in_progress' || status === 'dispatched') return '#f59e0b';
    if (status === 'resolved' || status === 'completed') return '#22c55e';
    return '#ef4444';
  };

  const isDispatched = selectedIncident.status === 'in_progress' || selectedIncident.status === 'dispatched';
  const isResolved = selectedIncident.status === 'resolved' || selectedIncident.status === 'completed';
    ...
    <div style={{
      ...
      <div>
        ...
        <div style={{ background: '#1e293b', padding: '16px', borderRadius: '8px', marginBottom: '16px' }}>
          <h3 style={{ margin: '0 0 8px 0', fontSize: '18px', color: '#38bdf8' }}>{selectedIncident.title || 'Emergency Incident'}</h3>
          <p style={{ margin: '4px 0', color: '#94a3b8', fontSize: '13px' }}>
            <strong>Type:</strong> {selectedIncident.request_type || 'SOS'}
          </p>
          <p style={{ margin: '4px 0', color: '#94a3b8', fontSize: '13px' }}>
            <strong>Priority Score:</strong> <span style={{ color: '#f87171', fontWeight: 'bold' }}>{selectedIncident.priority_score ?? 'N/A'}/100</span>
          </p>
          <p style={{ margin: '4px 0', color: '#94a3b8', fontSize: '13px' }}>
            <strong>People Affected:</strong> {selectedIncident.people_affected || 1}
          </p>
        </div>
        ...
        <div style={{ background: '#1e293b', padding: '16px', borderRadius: '8px' }}>
          ...
          <p style={{ margin: 0, fontSize: '13px', color: '#94a3b8', lineHeight: '1.4' }}>
            {selectedIncident.description || 'Emergency assistance requested.'}
          </p>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
        <button
          ...
          style={{
            padding: '12px',
            background: isDispatched || isResolved ? '#334155' : '#e11d48',
            color: '#fff',
            border: 'none',
            borderRadius: '6px',
            fontWeight: 'bold',
            cursor: isDispatched || isResolved ? 'not-allowed' : 'pointer',
          }}
        >
        ...
        <button
          ...
          style={{
            padding: '12px',
            background: isResolved ? '#334155' : '#16a34a',
            color: '#fff',
            border: 'none',
            borderRadius: '6px',
            fontWeight: 'bold',
