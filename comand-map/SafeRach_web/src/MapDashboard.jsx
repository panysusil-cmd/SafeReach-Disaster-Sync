...
export default function MapDashboard({ incidents = [], onSelectIncident }) {
  ...
  const filteredIncidents = incidents.filter((inc) => {
    if (filter === 'CRITICAL') return inc.priority_score >= 80 && inc.status !== 'completed';
    if (filter === 'ACTIVE') return inc.status !== 'completed';
    if (filter === 'COMPLETED') return inc.status === 'completed';
    return true;
  });
